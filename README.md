...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create brockdev-remix-2eda
  fly apps create brockdev-remix-2eda-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app brockdev-remix-2eda
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app brockdev-remix-2eda-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'brockdev-remix-2eda-staging' does not match app name in config file 'brockdev-remix-2eda'
  > ```
  >
